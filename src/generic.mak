...
ifeq ($(OBJS),)
CC_OBJCXX   = $(CC_SRC:.cxx=.o)
CC_OBJCPP   = $(CC_OBJCXX:.cpp=.o)
CC_OBJ      = $(CC_OBJCPP:.c=.o)
else
CC_OBJ      = $(sort $(OBJS) )
endif

ifeq ($(BUILD_PLATFORM),WINDOWS)
TEMP_FILES = $(subst /,\,$(wildcard $(addsuffix *.o,$(sort $(dir $(CC_OBJ)))))) $(wildcard *.ipk)
else
TEMP_FILES = $(wildcard *.o) $(CC_OBJ) $(wildcard *.ipk)
endif
...
%.o: %.cpp
	@echo compile $*.cpp
	$(CC) $(DEVICEOPTS) $(CC_FLAGS) $(CC_DEF) $(CC_INCDIRS) -c $*.cpp -o $*.o

%.o: %.cxx
	@echo compile $*.cxx
	$(CC) $(DEVICEOPTS) $(CC_FLAGS) $(CC_DEF) $(CC_INCDIRS) -c $*.cxx -o $*.o
    
%.o: %.c
	@echo compile $*.c
	$(CC) $(DEVICEOPTS) -x c++ $(CC_FLAGS) $(CC_DEF) $(CC_INCDIRS) -c $*.c -o $*.o
...
build_static: $(COMPILE)
	@echo build static $(CC_OBJ)
	$(AR) rcs $(OUTFILE) $(CC_OBJ)
...
clean_WINDOWS:
	...
	@cmd /C del $(TEMP_FILES)

clean_LINUX:
	@echo clean
	@rm -rf $(TARGETDIR)
	@rm -f $(OUTFILE) $(TEMP_FILES)
